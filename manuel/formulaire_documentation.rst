...
Pour importer massivement des documents relatifs aux photographies du projet, appuyer sur le bouton :guilabel:`Parcourir...` de :guilabel:`Import de photographies` dans la fenêtre *Imports* depuis la page d'accueil du projet, pour sélection le fichier csv d'import, puis appuyer sur le bouton :guilabel:`Importer` pour effectuer l'intégration.
Vous trouverez ensuite l'ensemble des données importées dans les rubriques *Documents*, onglet :guilabel:`photographie`.
